...
const create = (req, res) => {
    ...
    form.parse(req, async (err, fields, files) => {
        ...
        let auction = new Auction(originalValues)
        auction.seller = req.profile
        if (files.image) {
            auction.image.data = fs.readFileSync(files.image[0].filepath)
            auction.image.contentType = files.image[0].mimetype
        }
        try {
...
const update = (req, res) => {
    ...
    form.parse(req, async (err, fields, files) => {
        ...
        const originalValues = firstValues(form, fields)
        auction = lodash.extend(auction, originalValues)
...
const remove = async (req, res) => {
    try {
        let auction = req.auction
        let deletedAuction = auction.deleteOne()
